...
const abi = ["function gammaClaimed(uint256) public view returns (bool)"];
...
const BAKC: NextPage = () => {
  ...
  const checkClaimed = async (id, isOpensea) => {
    let claimed = await contract.gammaClaimed(parseInt(id));
    if (!claimed) {
  ...
  return (
    <Layout>
      ...
      >
        ...
        <Modal.Body>
          <p>Sorry, BAKC #{selectedTokenId} already used to claim ApeCoin.</p>
          <div>
            <Button
              variant="secondary"
